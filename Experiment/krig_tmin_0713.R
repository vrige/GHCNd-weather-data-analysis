tmin_0713 = TMIN_07.13_mean_krg
tmin_0713$value = tmin_0713$value/10
...
cord.dec = SpatialPoints(cbind(tmin_0713$LON, tmin_0713$LAT), proj4string = CRS("+proj=longlat +datum=WGS84"))
...
tmin_0713$x1 = cord.UTM$coords.x1
tmin_0713$x2 = cord.UTM$coords.x2
...
coordinates(tmin_0713) = c('x1','x2')

tmin_0713$dist<-sqrt((tmin_0713$x1-sea$x)^2+(tmin_0713$x2-sea$y)^2)
# coordinates(tmin.1)=c('LON','LAT')
# coordinates(tmin_1) = c('x1','x2')

# Estimate the variogram

svgm <- variogram(value ~ dist+ELEV, tmin_0713)
plot(svgm, main = 'Sample Variogram tmin 07-13',pch=19)
...
#plot(variogram(value ~ dist+ELEV, tmin_0713, alpha = c(0, 22, 45, 67, 90, 112, 135, 158)),main = "Variograms tmin 1967-1973",pch=19)
...
v.dir <- variogram(value~dist+ELEV,tmin_0713,alpha=c(0:3)*45)
plot(v.dir,pch=19)
v.anis <- vgm(2, "Gau", 400, 0.5, anis=c(90, 0.7))
...
# Fit the model
test = fit.variogram.reml(value~dist+ELEV, tmin_0713, model=v.anis)
test
...
usa_tbl %>% 
  ...
  geom_point(data = TMIN_07.13_mean_krg, aes(x=LON,y=LAT,color = value), size = 5) + scale_size(name="tmin")+
...
g.tr_tmin_0713 <- gstat(formula = value ~ dist+ELEV, data = tmin_0713, model = test)
...
# (x.min <- bbox(tmin_0713)[1,1]%/%res*res)
# (y.min <- bbox(tmin_0713)[2,1]%/%res*res)
# (x.max <- (bbox(tmin_0713)[1,2]+res)%/%res*res) # make sure it is outside the bbox
# (y.max <- (bbox(tmin_0713)[2,2]+res)%/%res*res) # make sure it is outside the bbox
# 
# x.min = 373.1241 -25
# x.max = 1592.729 +50
# 
...
# y.max = 1615.424 +25
# 
# grid_tmin_0713 <- expand.grid(x = seq(x.min, x.max, by=res),
#                               y = seq(y.min, y.max, by=res))
# grid_tmin_0713$dist<- sqrt((grid_tmin_0713$x-sea$x)^2+(grid_tmin_0713$y-sea$y)^2)
# grid_tmin_0713$ELEV<- df_elev_epqs$elevation



#real.grid_tmin_0713 <- na.omit(grid_tmin_0713)   # grid[-which(grid$ELEV=='NA'),]

# coordinates(real.grid_tmin_0713) <- c("x", "y")
# gridded(real.grid_tmin_0713) <- T; fullgrid(real.grid_tmin_0713) <- T

prova_tmin_0713 = as.data.frame(predict(g.tr_tmin_0713, grid, BLUE = FALSE))



# convert coords

cord.UTM = SpatialPoints(cbind(prova_tmin_0713$x, prova_tmin_0713$y), proj4string = CRS("+proj=lcc +lat_1=27.41666666666667 +lat_2=34.91666666666666 +lat_0=31.16666666666667 +lon_0=-100 +x_0=1000000 +y_0=1000000 +ellps=GRS80 +datum=NAD83 +units=km +no_defs "))

cord.dec <- spTransform(cord.UTM,CRS("+proj=longlat +datum=WGS84"))

cord.dec = as.data.frame(cord.dec)

prova_tmin_0713$LON = cord.dec$coords.x1
prova_tmin_0713$LAT = cord.dec$coords.x2
...
usa_tbl %>% 
  ...
  coord_map("ortho",orientation = c(39,-98,0))+
  geom_point(data = prova_tmin_0713, aes(x=LON,y=LAT,color = var1.var), size = 1) + scale_size(name="tmin")+
  scale_color_gradient(low="blue", high="yellow")